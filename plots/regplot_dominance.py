...
"""
This script describes the diversity of the data using index for dominance and
...
Usage:
    ...
    python3 regplot_dominance.py -df input.pkl -ft 0.4 -ct 10 -b 1000
...
Returns:
    The plot, saved into the file regplot_dom.pdf.
"""
...
data = posts.apply(lambda x:
                   diversity(x, 'dominance')).to_frame(name='Dominance')
...
# Plot the data
sns.regplot(x=data['Day'], y=data['Dominance'], data=data, scatter=False,
...
plt.ylabel('Dominance', fontsize=13)  # Set y-axis label

# Save the figure to PDF to the directory with the data
plt.savefig('regplot_dom.pdf', bbox_inches='tight')
