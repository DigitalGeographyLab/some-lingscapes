...
"""
This script describes the diversity of the data using Berger-Parker index and
uses pandas regression plot to render the results for visual inspection.

Usage:
    Execute the script from the command line using the following command:

    python3 regplot_berger-parker.py -df input.pkl -ft 0.4 -ct 10 -b 1000
...
Returns:
    The plot, saved into the file regplot_bp_dom.pdf.
"""
...
data = posts.apply(lambda x:
                   diversity(x, 'berger')).to_frame(name='Berger-Parker')
...
# Plot the data
sns.regplot(x=data['Day'], y=data['Berger-Parker'], data=data, scatter=False,
...
plt.ylabel('Berger-Parker dominance index', fontsize=13)  # Set y-axis label

# Save the figure to PDF to the directory with the data
plt.savefig('regplot_bp_dom.pdf', bbox_inches='tight')
