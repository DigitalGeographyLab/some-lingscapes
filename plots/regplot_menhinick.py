...
"""
This script describes the diversity of the data using Menhinick's richness index
and uses pandas regression plot to render the results for visual inspection.

Usage:
    Execute the script from the command line using the following command:

    python3 regplot_menhinick.py -df input.pkl -ft 0.4 -ct 10 -b 1000
...
Returns:
    The plot, saved into the file regplot_menh.pdf.
"""
...
data = posts.apply(lambda x:
                   diversity(x, 'menhinick')).to_frame(name='Menhinick')
...
# Plot the data
sns.regplot(x=data['Day'], y=data['Menhinick'], data=data, scatter=False,
...
plt.ylabel('Menhinick richness index', fontsize=13)  # Set y-axis label

# Save the figure to PDF to the directory with the data
plt.savefig('regplot_menh.pdf', bbox_inches='tight')
