...
to plot the results against the number of singletons, or single occurrences of
a language per day.

Usage:
    Execute the script from the command line using the following command:

    python3 regplot_richness.py -df input.pkl -ft 0.4 -ct 10 -b 1000
...
Returns:
    The plot, saved into the file regplot_rich.pdf.
"""
...
data = posts.apply(lambda x: diversity(x, 'unique')).to_frame(name='Richness')

# Next, count singletons and append them to the DataFrame.
data['Singletons'] = posts.apply(lambda x: diversity(x, 'singles'))
...
# Plot the data
sns.regplot(x=data['Day'], y=data['Richness'], data=data, scatter=False,
            fit_reg=True, x_ci=99.9, order=3, n_boot=n_boot, label="Languages")

sns.regplot(x=data['Day'], y=data['Singletons'], data=data, scatter=False,
